...
await page.evaluate((o) => { const g = window.game; g.api.step(30); g.controller.camPitch = o.pitch; g.controller.camDist = o.dist; if (o.front) g.controller.camYaw = g.controller.yaw + Math.PI; if (o.side) g.controller.camYaw = g.controller.yaw + Math.PI / 2; if (o.spawn) { g.api.spawnAnimal('machairodus', 3, 1); g.api.spawnAnimal('antelope', -3, 2); } if (o.walk) { const yaw0 = g.controller.camYaw; g.controller.camYaw = g.controller.yaw; g.api.press('forward'); g.api.step(25); g.controller.camYaw = yaw0 + (o.side ? 0 : 0); if (o.side) g.controller.camYaw = g.controller.yaw + Math.PI / 2; } else g.api.step(2); for (let i = 0; i < (o.side || o.front ? 30 : 2); i++) g.render(0.05); g.updateHud(); }, opts);
